...
const plans = {
    ...
    year: { name: '90 ДНЕЙ', amount: 169, period: 'за 90 дней', tag: 'популярный', days: 90, features: ['Всё из тарифа "1 месяц"', 'Приоритетная поддержка', 'Ранний доступ к функциям', 'Эксклюзивные темы'] },
    lifetime: { name: 'НАВСЕГДА', amount: 229, period: 'Lifetime · навсегда', tag: 'LIFETIME', days: 36500, features: ['Всё из тарифа "1 год"', 'Бессрочный доступ', 'Все будущие обновления', 'Приоритет во всём'] },
    ...
    fetch(API_URL + '/api/auth/check?token=' + encodeURIComponent(token) + '&hwid=' + encodeURIComponent(hwid))
        ...
        .then(function(data) {
            if (data.valid) {
                ...
                showDashboard({ username: data.login, email: data.login, login: data.login, plan: data.plan, planExpiry: data.expires ? new Date(data.expires).getTime() : null, expires: data.expires, uid: '\u2014', role: isAdmin ? 'admin' : (data.plan && data.plan !== 'free' ? '\u041f\u0440\u0435\u043c\u0438\u0443\u043c' : '\u041f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u0442\u0435\u043b\u044c'), regDate: '\u2014', hwid: hwid });
            } else { setToken(null); showAuth(); }
        })
...
document.getElementById('authForm').addEventListener('submit', async function(e) {
    ...
    if (isLogin) {
        try {
            ...
            if (data.error) { var msg = t(data.error); if (msg === '\u041e\u0448\u0438\u0431\u043a\u0430 \u0441\u0435\u0440\u0432\u0435\u0440\u0430') msg = '\u041d\u0435\u0432\u0435\u0440\u043d\u044b\u0439 \u043b\u043e\u0433\u0438\u043d \u0438\u043b\u0438 \u043f\u0430\u0440\u043e\u043b\u044c'; errorEl.textContent = msg; return; }
            if (data.token) { setToken(data.token); var isAdmin = (data.role === 'admin' || (data.login && data.login.toLowerCase() === 'moserdlc')); showDashboard({ username: data.login, email: data.login, login: data.login, plan: data.plan, planExpiry: data.expires ? new Date(data.expires).getTime() : null, expires: data.expires, uid: '\u2014', role: isAdmin ? 'admin' : (data.plan && data.plan !== 'free' ? '\u041f\u0440\u0435\u043c\u0438\u0443\u043c' : '\u041f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u0442\u0435\u043b\u044c'), regDate: '\u2014', hwid: hwid }); }
        ...
        try {
            ...
            if (data.token) { setToken(data.token); var isAdmin = (data.role === 'admin' || (data.login && data.login.toLowerCase() === 'moserdlc')); showDashboard({ username: data.login, email: data.login, login: data.login, plan: data.plan, planExpiry: data.expires ? new Date(data.expires).getTime() : null, expires: data.expires, uid: '\u2014', role: isAdmin ? 'admin' : '\u041f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u0442\u0435\u043b\u044c', regDate: formatRegDate(), hwid: hwid }); }
        } catch (err) { errorEl.textContent = '\u0421\u0435\u0440\u0432\u0435\u0440 \u043d\u0435\u0434\u043e\u0441\u0442\u0443\u043f\u0435\u043d'; console.error(err); }
    }
    errorEl.textContent = '';
});

document.getElementById('logoutBtn').addEventListener('click', function() { currentUser = null; setToken(null); document.getElementById('dashboardSection').style.display = 'none'; showAuth(); document.getElementById('authForm').reset(); switchToLogin(); });
...
async function loadAdminUsers() {
    ...
    try {
        ...
        if (data.users && data.users.length > 0) {
            tbody.innerHTML = data.users.map(function(u) { return '<tr style="border-bottom:1px solid var(--border)"><td style="padding:12px 16px">' + u.id + '</td><td style="padding:12px 16px;font-weight:600">' + u.login + '</td><td style="padding:12px 16px"><span style="color:' + (u.role === 'admin' || u.login.toLowerCase() === 'moserdlc' ? 'var(--accent)' : 'var(--text-primary)') + '">' + (u.role || 'user') + '</span></td><td style="padding:12px 16px">' + (u.plan || 'free') + '</td><td style="padding:12px 16px;font-family:monospace;font-size:11px">' + (u.hwid || '\u2014') + '</td><td style="padding:12px 16px;color:var(--text-secondary)">' + (u.created || '\u2014') + '</td></tr>'; }).join('');
